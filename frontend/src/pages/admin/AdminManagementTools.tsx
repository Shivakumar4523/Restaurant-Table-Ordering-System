...
export type AdminManagementTab = "menu" | "categories" | "offers" | "tables" | "employees";

export const adminManagementTabs: AdminManagementTab[] = ["menu", "categories", "offers", "tables"];
...
export function AdminManagementTools({ tab }: { tab: AdminManagementTab }) {
  ...
  async function load() {
    ...
    try {
      const [categoryData, menuData, couponData, tableData, employeeData] = await Promise.all([
        getCategories(),
        getMenuItems(),
        getCoupons(),
        getTables(),
      ...
      setMessage(error instanceof Error ? error.message : "Unable to refresh management data.");
    } finally {
      setLoading(false);
    }
  }
